...
export let initAchievementList = function (achievements : { [index : string] : {name : string, desc : string, hidden? : boolean} }) {
    ...
    for (let id in achievements) {
        ...
        $a.find('.twitter').attr('href', 'http://twitter.com/share?url=http%3A%2F%2Fstardust.gg&text=I%20unlocked%20the%20%27'+ achievement.name +'%27%20achievement%20on%20Stardust%27s%20%23BlockchainQuest%21');
        $a.show();
